...
public func humanReadable(_ error: AcervoError) -> String {
    ...
    case .manifestDownloadFailed(let statusCode):
        return "Error: Failed to download the CDN manifest (HTTP \(statusCode)). Check your network connection and try again."

    case .manifestIntegrityFailed(let expected, let actual):
        return "Error: CDN manifest integrity check failed (expected '\(expected)', got '\(actual)'). The manifest may be corrupted; try again."

    case .downloadFailed(let fileName, let statusCode):
        return "Error: Download failed for '\(fileName)' (HTTP \(statusCode)). Check your network connection and try again."

    case .integrityCheckFailed(let file, let expected, let actual):
        return "Error: Integrity check failed for '\(file)' (expected SHA-256 '\(expected)', got '\(actual)'). The file may be corrupted; try re-downloading."

    case .downloadSizeMismatch(let fileName, let expected, let actual):
        return "Error: Downloaded file '\(fileName)' has unexpected size (expected \(expected) bytes, got \(actual) bytes). Try re-downloading."
    ...
    case .componentNotRegistered(let componentId):
        return "Error: Component '\(componentId)' is not registered. Use 'bruja list' to see available components."
    ...
    case .invalidModelId(let modelId):
        return "Error: Invalid model ID '\(modelId)'. Expected format: 'org/repo' (e.g., mlx-community/Llama-3.2-1B-Instruct-4bit)."

    case .componentNotDownloaded(let componentId):
        return "Error: Component '\(componentId)' has not been downloaded. Run 'bruja download -m \(componentId)' first."

    case .componentFileNotFound(let component, let file):
        return "Error: File '\(file)' not found in component '\(component)'. Try re-downloading the model."
    ...
    case .manifestVersionUnsupported(let version):
        return "Error: CDN manifest version \(version) is not supported by this version of bruja. Please update bruja."

    case .manifestModelIdMismatch(let expected, let actual):
        return "Error: CDN manifest model ID mismatch (expected '\(expected)', got '\(actual)'). The manifest may be for a different model."
